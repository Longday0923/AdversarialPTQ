...
att_type="PGD"
att_tar="untar"
STEPSIZE=(0.1 0.01 0.003) # 0.05 in HW, 0.003 in TRADES
...
# ----------------------------------------------------------------
# for each_numrun in {1..5..1}; do       # it runs 10 times
each_numrun=1
for each_lratio in ${LRATIOS[@]}; do
for each_margin in ${MARGINS[@]}; do
for att_step_size in ${STEPSIZE[@]}; do

  # : make-up random-seed
  randseed=$((215+10*each_numrun))

  # : run scripts
  echo "======================= step-size = $att_step_size ======================="
